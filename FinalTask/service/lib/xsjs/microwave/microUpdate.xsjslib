...
function microUpdate(param) {
  ...
  var uStmt;
  uStmt = param.connection.prepareStatement(`UPDATE "${MICRO_TABLE}" SET "brand"='${oMicro.brand}',"color"='${oMicro.color}',"ts_update" = ${CURR_TIMESTAMP_FUN} WHERE "microid"=${oMicro.microid};`);
  uStmt.executeUpdate();
}
